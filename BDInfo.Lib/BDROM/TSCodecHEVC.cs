...
namespace BDInfo.Lib.BDROM
{
    public abstract class TSCodecHEVC
    {
        ...
        public static void Scan(TSVideoStream stream, TSStreamBuffer buffer, ref string tag, bool extendedStreamDiagnostics)
        {
            ...
            if (extendedStreamDiagnostics && !stream.IsInitialized)
            {
        ...
        private static void VideoParameterSet(TSStreamBuffer buffer)
        {
            ...
            for (var subLayerPos = (tempB ? 0 : maxSubLayers); subLayerPos <= maxSubLayers; subLayerPos++)
            {
                buffer.SkipExpMulti(3,
                    true); //vps_max_dec_pic_buffering_minus1, vps_max_num_reorder_pics, vps_max_latency_increase_plus1
            }
            ...
            if (vpsTimingInfoPresentFlag)
            {
                ...
                if (vpsNumHRDParameters > 1024)
                {
                    vpsNumHRDParameters = 0;
                }
        ...
        private static void PicParameterSet(TSStreamBuffer buffer)
        {
            ...
            PicParameterSets[(int) ppsPicParameterSetID] = new PicParameterSetStruct(
        ...
        private static void SeiMessageBufferingPeriod(TSStreamBuffer buffer, ref uint seqParameterSetID, uint payloadSize)
        {
            ...
            {
                SeiMessageBufferingPeriodXXL(buffer, seqParameterSetItem.VUIParameters?.XXLCommon,
                    irapCPBParamsPresentFlag, seqParameterSetItem.VUIParameters?.NAL, payloadSize);
            }

            if (seqParameterSetItem.VclHrdPbPresentFlag)
            {
                SeiMessageBufferingPeriodXXL(buffer, seqParameterSetItem.VUIParameters?.XXLCommon,
                    irapCPBParamsPresentFlag, seqParameterSetItem.VUIParameters?.VCL, payloadSize);
            }
        }
        ...
        private static void HRDParameters(
